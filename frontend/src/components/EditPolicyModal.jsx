...
const EditPolicyModal = ({ policy, onClose, onUpdated }) => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      issuedDate: isIssued && form.issuedDate ? form.issuedDate.toISOString().split("T")[0] : "",
    ...
    try {
      setLoading(true);
      await axios.put(`http://localhost:5000/api/policies/${form.applicationNumber}`, payload);
      onUpdated();
      handleClose();
    } catch (err) {
      console.error(err);
